class Solution {
public:
    string helper(vector<pair<int,string>> &mp,int num){
        for(auto it:mp){
            if(num/it.first>0){
                return ((num>=100?helper(mp,num/it.first)+" ":"")+it.second+(num%it.first==0?"":" ")+helper(mp,num%it.first));
            }
        }
        return "";
    }
    ...
    string numberToWords(int num) {
        ...
        vector<pair<int,string>> mp={{1000000000,"Billion"},{1000000,"Million"},{1000,"Thousand"},{100,"Hundred"},
